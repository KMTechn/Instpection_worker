...
class InspectionProgram:
    ...
    def __init__(self):
        ...
        self.processed_defect_labels: List[Dict[str, Any]] = []
    ...
    def _create_defective_view(self, container):
        ...
        left_frame.grid_rowconfigure(1, weight=1) # 미처리 목록
        left_frame.grid_rowconfigure(3, weight=1) # 처리된 목록
        left_frame.grid_columnconfigure(0, weight=1)

        # 미처리 불량품 목록
        ttk.Label(left_frame, text="[미처리] 불량품 목록", style='TLabel', font=(self.DEFAULT_FONT, int(12 * self.scale_factor), 'bold')).grid(row=0, column=0, sticky='w')
        unprocessed_cols = ('item_name', 'item_code', 'worker', 'count')
        self.unprocessed_defects_tree = ttk.Treeview(left_frame, columns=unprocessed_cols, show='headings')
        self.unprocessed_defects_tree.grid(row=1, column=0, sticky='nsew', pady=(5, 10))
        ...
        # 생성된 불량표 목록
        ttk.Label(left_frame, text="[생성 완료] 불량표 목록", style='TLabel', font=(self.DEFAULT_FONT, int(12 * self.scale_factor), 'bold')).grid(row=2, column=0, sticky='w', pady=(10, 0))
        processed_cols = ('defect_id', 'item_name', 'quantity', 'creation_date')
        self.processed_labels_tree = ttk.Treeview(left_frame, columns=processed_cols, show='headings')
        self.processed_labels_tree.grid(row=3, column=0, sticky='nsew', pady=(5, 0))
        self.processed_labels_tree.heading('defect_id', text='불량표 ID')
        self.processed_labels_tree.heading('item_name', text='품목명')
        self.processed_labels_tree.heading('quantity', text='수량')
        self.processed_labels_tree.heading('creation_date', text='생성일')
        ...
        self.start_defect_merge_button = ttk.Button(session_ctrl_frame, text="불량 합치기 시작", command=self.start_defective_merge_session, state=tk.NORMAL)
        self.start_defect_merge_button.grid(row=2, column=0, pady=5, sticky='w')

        self.direct_defect_entry_button = ttk.Button(session_ctrl_frame, text="불량 직접 입력", command=self._prompt_direct_defect_input)
        self.direct_defect_entry_button.grid(row=2, column=1, pady=5, sticky='w', padx=(5,0))
    ...
    def on_available_defect_select(self, event=None):
        # 이 함수는 이제 선택 시각적 피드백 외에 특별한 로직을 수행하지 않습니다.
        # 버튼 활성화 로직은 제거되었습니다.
        pass
    ...
    def load_all_defective_items(self):
        ...
        unprocessed_defects = {}
        processed_defect_labels = []
        reworked_barcodes = set()

        # 1. 이미 불량표로 생성된 (처리된) 불량품 목록을 먼저 가져옵니다.
        processed_barcodes = set()
        if os.path.exists(self.defects_data_folder):
            for date_folder in os.listdir(self.defects_data_folder):
                date_path = os.path.join(self.defects_data_folder, date_folder)
                if os.path.isdir(date_path):
                    for label_file in os.listdir(date_path):
                        if label_file.endswith('.json'):
                            try:
                                with open(os.path.join(date_path, label_file), 'r', encoding='utf-8') as f:
                                    label_data = json.load(f)
                                    processed_defect_labels.append(label_data)
                                    processed_barcodes.update(label_data.get('barcodes', []))
                            except Exception as e:
                                print(f"불량표 데이터 파일 '{label_file}' 처리 중 오류: {e}")

        log_folder = self.save_folder

        # 2. 리워크된 불량품 목록을 가져옵니다.
        ...
        # 3. 모든 검사 로그를 순회하며 '미처리' 불량품을 식별합니다.
                    ...
                    with open(os.path.join(log_folder, filename), 'r', encoding='utf-8-sig') as f:
                        ...
                        for row in reader:
                            if row.get('event') == 'INSPECTION_DEFECTIVE':
                                ...
                                # 이미 처리되었거나 리워크된 불량품은 제외
                                if barcode in processed_barcodes or barcode in reworked_barcodes:
                                    continue
                                ...
                                if item_code_from_barcode:
                                    ...
                                    unprocessed_defects[defect_key]['barcodes'].add(barcode)
                except Exception as e:
                    print(f"검사 로그 파일 '{filename}' 처리 중 오류: {e}")

        self.unprocessed_defects = unprocessed_defects
        self.processed_defect_labels = processed_defect_labels
    ...
    def _update_defective_mode_ui(self):
        ...
        # 2. 생성된 불량표 목록 업데이트
        for i in self.processed_labels_tree.get_children():
            self.processed_labels_tree.delete(i)

        sorted_processed = sorted(self.processed_defect_labels, key=lambda item: item.get('creation_date', ''), reverse=True)
        for label_data in sorted_processed:
            self.processed_labels_tree.insert('', 'end', values=(
                label_data.get('defect_box_id', ''),
                label_data.get('item_name', ''),
                label_data.get('quantity', ''),
                label_data.get('creation_date', '').split('T')[0] # 날짜만 표시
            ))
    ...
    def _prompt_direct_defect_input(self):
        """현품표 없이 불량품을 직접 입력받기 위한 팝업창을 띄웁니다."""
        popup = tk.Toplevel(self.root)
        popup.title("불량 직접 입력")
        ...
        ttk.Label(main_frame, text="불량을 등록할 품목의 코드를 입력하거나 스캔하세요.", font=(self.DEFAULT_FONT, 11)).pack(pady=5)
        item_code_entry = tk.Entry(main_frame, font=(self.DEFAULT_FONT, 12, 'bold'), justify='center')
        item_code_entry.pack(fill=tk.X, ipady=5, pady=(0, 15))
        item_code_entry.focus()

        ttk.Label(main_frame, text="불량 처리할 제품의 바코드를 입력하거나 스캔하세요.", font=(self.DEFAULT_FONT, 11)).pack(pady=5)
        barcode_entry = tk.Entry(main_frame, font=(self.DEFAULT_FONT, 12, 'bold'), justify='center')
        barcode_entry.pack(fill=tk.X, ipady=5, pady=(0, 20))

        def on_confirm():
            item_code = item_code_entry.get().strip()
            barcode = barcode_entry.get().strip()
            if not item_code or not barcode:
                messagebox.showwarning("입력 오류", "품목 코드와 제품 바코드를 모두 입력해야 합니다.", parent=popup)
                return

            # 품목 유효성 검사
            matched_item = next((item for item in self.items_data if item['Item Code'] == item_code), None)
            if not matched_item:
                messagebox.showwarning("품목 오류", f"입력한 품목코드 '{item_code}'를 찾을 수 없습니다.", parent=popup)
                return

            popup.destroy()
            self._handle_direct_defect_scan(item_code, barcode)

        confirm_button = ttk.Button(main_frame, text="등록", command=on_confirm)
        confirm_button.pack(ipady=10, fill=tk.X)

        item_code_entry.bind('<Return>', lambda e: barcode_entry.focus())
        barcode_entry.bind('<Return>', lambda e: on_confirm())

    def _handle_direct_defect_scan(self, item_code: str, barcode: str):
        """직접 입력된 불량 정보를 처리하고 로그를 기록합니다."""
        matched_item = next((item for item in self.items_data if item['Item Code'] == item_code), None)
        if not matched_item:
            # 이중 확인, 보통은 prompt에서 걸러짐
            self.show_fullscreen_warning("품목 없음", f"품목코드 '{item_code}' 정보를 찾을 수 없습니다.", self.COLOR_DEFECT)
            return
        ...
        self.show_status_message(f"불량품 '{barcode}'가 직접 등록되었습니다.", self.COLOR_SUCCESS)
        self.load_all_defective_items() # 목록 새로고침
